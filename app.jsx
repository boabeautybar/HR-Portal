...
function App({ currentUser, onSignOut, appUsers, onUsersUpdate }) {
  ...
  // Per-day per-ec early-leave entries from the check-in kiosk's "Left early"
  // feature. Shape: { [dayKey]: { [ec]: { hours, recordedAt, recordedBy } } }.
  // Loaded alongside the attendance grid; subtracted from each tech's day
  // total and surfaced as a "−Xh" badge on the relevant cells.
        ...
        try {
          ...
          for (const s of bStaff) {
            for (const dy of daysX) {
              ...
              const kioskMarkedAbsent = !!kioskAbs && kioskAbs.status !== "ext" && kioskAbs.status !== "trial" && kioskAbs.status !== "swap_o" && kioskAbs.status !== "left_early" && kioskAbs.status !== "(left_early)";
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Early-leave sidecar — guard against older data.js without the helper.
      safe(window.BOA_DB.loadEarlyLeaves ? window.BOA_DB.loadEarlyLeaves(attBranch, attYM) : Promise.resolve({}))
...
{/* Per-branch cards */}
        ...
        {tab==="attendance" && !((currentUser.hideCategories || []).includes("Payroll")) && (() => {
          ...
          const totalsFor = (ec) => {
            ...
            // Early-leave hours (from the check-in kiosk's "Left early" tag).
            // Treated as unpaid hours so they flow through the same payroll
            // accounting (totalUnpaid / unpaidAfterExd) as deduct:N cells.
            t.earlyHours = 0;
            for (const _dk in attEarly) {
              const _entry = attEarly[_dk] && attEarly[_dk][ec];
              if (_entry && _entry.hours) t.earlyHours += Number(_entry.hours) || 0;
            }
            t.unpaidHours += t.earlyHours;
                      ...
                      const dataRow = (
                        <tr key={s.ec} style={isMgr ? { background:"#fffaf0" } : undefined}>
                          ...
                          {days.map(dy => {
                            ...
                            // Early-leave entry for this (tech, day), if any. Drives a
                            // centered "Left Xh early" indicator + admin-review warning.
                            const earlyRec = (attEarly && attEarly[dy.d] && attEarly[dy.d][s.ec]) || null;
                            const earlyHours = earlyRec && earlyRec.hours ? earlyRec.hours : 0;
                            const isHol = !!(holidayLookup && holidayLookup[dy.ymd]);
                            ...
                            const scheduleSaysWork    = hint === "on" || hint === "ext";
                            // Note: an early-leave kiosk entry isn't an absence — the tech worked,
                            // just not the full day. We surface that via the dedicated early-leave
                            // warning below instead of the absent-schedule-mismatch tooltip.
                            const kioskAbsentScheduled = !!kioskAbs && scheduleSaysWork && kioskAbs.status !== "left_early" && kioskAbs.status !== "(left_early)";
                            ...
                            const kioskMarkedAbsent = !!kioskAbs && kioskAbs.status !== "ext" && kioskAbs.status !== "trial" && kioskAbs.status !== "swap_o" && kioskAbs.status !== "left_early" && kioskAbs.status !== "(left_early)";        // kiosk audit log entry, only true non-presence statuses
                            ...
                            const ttl =
                              ...
                              (earlyHours ? "\n🏃 Left " + earlyHours + "h early"
                                + (earlyRec.recordedBy ? " · recorded by " + earlyRec.recordedBy : "")
                                + (earlyRec.recordedAt ? " · " + new Date(earlyRec.recordedAt).toLocaleString("en-ZA", { hour:"2-digit", minute:"2-digit", day:"2-digit", month:"short" }) : "")
                                + " · " + earlyHours + "h deducted from payroll" : "") +
                              ...
                              <td key={dy.d} style={{ padding:0, borderBottom:"1px solid #FCE7F3", borderLeft: allMatchEdge, background: allMatchBg || cellBaseBg, position:"relative" }}>
                                <div style={{ position:"relative", height:36 }}>
                                  ...
                                  {(() => {
                                    ...
                                    // Early-leave needs admin review until acknowledged — it's a
                                    // payroll-affecting record and shouldn't ship to payroll without
                                    // being seen.
                                    const earlyLeaveNeedsReview = !!earlyHours;
                                    const warning  = apptVsKioskAbsentWarn || presentNoApptWarn || extDayNoApptWarn || missingCheckin || absentNeedsReview || earlyLeaveNeedsReview;
                                    ...
                                    if (warning && !reviewed) {
                                      const warnTitle = apptVsKioskAbsentWarn ? "⚠ Kiosk marked tech absent but Fresha has a completed appointment that day"
                                                      ...
                                                      : earlyLeaveNeedsReview  ? ("⚠ Left " + earlyHours + "h early — admin must review the deduction"
                                                                                  + (earlyRec.recordedBy ? " (recorded by " + earlyRec.recordedBy + ")" : "")
                                                                                  + " before payroll")
                                  ...
                                  {v && (
                                    <div style={{ position:"absolute", top:6, bottom: earlyHours > 0 ? (s.role === "NT" ? 20 : 14) : (s.role === "NT" ? 6 : 0), left:0, right:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontStyle: (override && !isFutureSwap) ? "normal" : "italic", fontWeight: (override && !isFutureSwap) ? 700 : 400, color: isFutureSwap ? "#9ca3af" : (override ? st.fg : (hintFg + "70")), pointerEvents:"none", letterSpacing:"0.02em" }}>{st.lbl || hintLbl || ""}</div>
                                  )}
                                  {/* Early-leave indicator from the kiosk's "Left early" feature.
                                      Drawn as a centered orange chip on the lower half of the cell so
                                      "On Time" stays readable at the top and "Left Xh early" reads at
                                      a glance below. The chip is z-index 4 so it always sits above the
                                      hidden <select> overlay and stays visible after the cell tooltip
                                      opens. The red ⚠ at top-right is wired separately to require
                                      admin review before payroll. */}
                                  {earlyHours > 0 && (
                                    <div style={{ position:"absolute", left:0, right:0, bottom: s.role === "NT" ? 8 : 2, display:"flex", justifyContent:"center", pointerEvents:"none", zIndex:4 }}>
                                      <span style={{ fontSize:9, lineHeight:1.05, fontWeight:800, color:"#7C2D12", background:"#FED7AA", border:"1px solid #EA580C", borderRadius:4, padding:"1px 4px", letterSpacing:"0.02em", whiteSpace:"nowrap", textShadow:"none", boxShadow:"0 0 2px rgba(255,255,255,0.6)" }}>
                                        🏃 −{earlyHours}h early
                                      </span>
                                    </div>
                                  )}
