...
// ─── MANAGER MODAL ────────────────────────────────────────────────────────────────
function ManagerModal({ m, onClose, onSave, onDelete }) {
...
function App() {
  ...
  const tabBtn = (t,label) => (
    <button key={t} onClick={()=>setTab(t)} style={{ padding:"10px 18px", borderRadius:14, cursor:"pointer", fontFamily:"inherit", fontWeight:700, fontSize:13, border:"none", background:tab===t?"#BE185D":"#FFFFFF", color:tab===t?"#FFFFFF":"#831843", boxShadow:tab===t?"0 4px 12px rgba(190,24,93,0.32)":"0 2px 6px rgba(0,0,0,0.06)", transition:"all .18s", margin:"4px 4px" }}>{label}</button>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:cream, fontFamily:"'DM Sans',sans-serif", color:"#831843" }}>
      ...
      <div style={{ maxWidth:1380, margin:"0 auto", padding:"22px 24px" }}>
        ...
        {tab==="schedules" && <Schedule allStaff={enriched} />}
...
{/* Per-branch cards */}
        ...
        {tab==="leave" && (() => {
          ...
          const ym = leaveYM;
          const f  = leaveForm;
          // Active nail techs at this branch — exclude maternity, off-boarded, managers
          const allTechs = enriched
            .filter(s => s.branch === br && !s.onMat && /^[BT]/.test(s.ec) && !s.offHidden)
            .sort((a,b) => (a.ec || "").localeCompare(b.ec || ""));
          const maxLeave = Math.max(1, Math.floor(allTechs.length * 0.2));
          ...
          const annualCount = (iso) => {
            ...
            for (const lv of leaveRecs) {
              ...
              const s2 = enriched.find(x => x.ec === lv.ec);
              if (!s2 || s2.branch !== br || s2.onMat || !/^[BT]/.test(s2.ec)) continue;
          ...
          const addLeave = () => {
            if (!f.ec || !f.startDate || !f.endDate) { alert("Please fill in nail tech, from, and to dates."); return; }
            if (new Date(f.startDate) > new Date(f.endDate)) { alert("Start date must be on or before end date."); return; }
            const stf = enriched.find(s => s.ec === f.ec);
            if (!stf || stf.onMat || !/^[BT]/.test(stf.ec)) { alert("Leave Planner manages annual leave for nail techs (B/T staff) only."); return; }
            const stBr = stf.branch;
            const stTechs = enriched.filter(s => s.branch === stBr && !s.onMat && /^[BT]/.test(s.ec) && !s.offHidden);
            const stMx = Math.max(1, Math.floor(stTechs.length * 0.2));
            ...
            for (let d = new Date(sd); d <= ed; d.setDate(d.getDate()+1)) {
              ...
              for (const lv of leaveRecs) {
                if (lv.type !== "Annual leave") continue;
                const ls = enriched.find(s2 => s2.ec === lv.ec);
                if (!ls || ls.branch !== stBr || ls.onMat || !/^[BT]/.test(ls.ec)) continue;
                if (ds >= lv.startDate && ds <= lv.endDate) ct++;
              }
              if (ct > stMx) {
                alert("Cannot add: " + ct + " nail techs would be on annual leave on " + ds + ". Max " + stMx + " allowed (20% of " + stTechs.length + " nail techs at " + stBr + ").");
                return;
              }
            }
          ...
          // Records list, filtered to this branch
          const storeLeave = leaveRecs
            .filter(lv => lv.type === "Annual leave" && (enriched.find(s => s.ec === lv.ec) || {}).branch === br)
            .slice()
            ...
            <div>
              ...
              <div style={{ background:"#FFFFFF", borderRadius:13, padding:"14px 16px", border:"1px solid " + Y, marginBottom:14, display:"flex", flexWrap:"wrap", gap:14, alignItems:"flex-end" }}>
                ...
                <div style={{ flex:1, minWidth:240, fontSize:12, color:"#831843", lineHeight:1.5 }}>
                  <div><strong>{allTechs.length}</strong> active nail tech{allTechs.length !== 1 ? "s" : ""} at <strong>{br}</strong> · cap: <strong style={{ color:"#F472B6" }}>max {maxLeave} on annual leave per day</strong> (20%)</div>
              ...
              <div style={{ background:"#FFFFFF", borderRadius:13, padding:"14px 16px", border:"1px solid " + Y, marginBottom:14 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1.5fr 1fr 1fr auto", gap:8, marginBottom:10, alignItems:"flex-end" }}>
                  <div>
                    <label style={{ fontSize:10, color:"#F472B6", fontWeight:700 }}>NAIL TECH</label>
                    <select value={f.ec} onChange={e=>setLeaveForm({...f, ec:e.target.value})} style={{ width:"100%", padding:"6px 9px", borderRadius:6, border:"1px solid " + Y, fontFamily:"inherit", fontSize:12, background:aA }}>
                      <option value="">— select —</option>
                      {enriched.filter(s => !s.onMat && /^[BT]/.test(s.ec) && !s.offHidden)
                        .sort((a,b) => (a.ec || "").localeCompare(b.ec || ""))
                        .map(z => <option key={z.ec} value={z.ec}>{z.ec} · {z.name} ({z.branch})</option>)}
                    </select>
                  </div>
                  <div>
              ...
              <div style={{ background:"#FFFFFF", borderRadius:11, border:"1px solid " + Y, marginBottom:14, overflow:"auto" }}>
                ...
                <table style={{ borderCollapse:"collapse", fontSize:10, fontFamily:"inherit" }}>
                  <thead>
                    <tr>
                      <th style={{ padding:"6px 8px", textAlign:"left", position:"sticky", left:0, background:aA, zIndex:2, minWidth:160, fontSize:9, color:"#F472B6", borderBottom:"1px solid " + Y, borderRight:"2px solid " + Y }}>NAIL TECH</th>
                      {monthLabels.map(ml => (
      ...
      {mgrModal && <ManagerModal m={mgrModal} onClose={()=>setMgrModal(null)} onSave={saveMgr} onDelete={delMgr} />}
