/* ============================================================
   BOA — shared manager/tech shift-hour rules (single source of truth).

   ONE implementation of the per-store × role × day-of-week × shift-code
   opening/closing hours. Until now the SAME rule set was copied verbatim
   ...
   so a rule change happens HERE, once.

   Loaded as a plain <script> before every surface that renders shift hours:
     index.html            → shift-rules.js  (root, before app.jsx)
     kiosk/index.html      → ../shift-rules.js (before staff-app/manager-app)
     myboa/schedule.html   → ../shift-rules.js (before schedule.js)
