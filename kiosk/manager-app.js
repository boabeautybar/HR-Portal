...
(function () {
  ...
  function renderManagerLanding() {
    ...
    setMain(
      ...
      '<div class="tile-grid tile-grid-4">' +
        ...
        '</button>' +
        '<button class="tile tile-big" id="tile-borrow" type="button">' +
          '<div class="tile-icon">🔀</div>' +
          '<div class="tile-label">Borrow Tech Today</div>' +
          '<div class="tile-hint">ONE-DAY GUEST</div>' +
        '</button>' +
      '</div>'
    );
    ...
    document.getElementById("tile-borrow").onclick   = function () { renderBorrowTech(); };
  }

  // ---------------- Borrow Tech Today ----------------
  // Walk-in flow: a tech from another store is here for the day. The
  // manager picks her from a searchable list, taps Borrow, and a record
  // is written to boa_tech_loans_v1. From then on this kiosk's roster
  // (via PR #70) and the HR portal's Attendance grid (via PR #71)
  // automatically reflect the move - hours stay attributed to her home
  // store; she check-ins here.
  async function renderBorrowTech() {
    ...
    var todayLabel = today.toLocaleDateString("en-ZA", { weekday: "long", day: "2-digit", month: "long" });
    var thisBranch = cfg.branchName || "";

    setMain(
      '<div class="hero">' +
        '<div class="hero-brand">🔀 Borrow a Tech for Today</div>' +
        '<div class="hero-sub" style="margin-top:6px;font-size:13px;color:var(--gray-600)">' + esc(todayLabel) + ' · ' + esc(cfg.branchDisplayName || thisBranch) + '</div>' +
      '</div>' +
      '<div class="card" style="max-width:680px;margin:14px auto 0;padding:16px 18px;background:#FFFBEB;border:1px solid #FDE68A;border-radius:14px">' +
        '<div style="font-size:12px;color:#78350F;line-height:1.5">' +
          'A tech from another store is here today? Pick her below — her name will appear on this kiosk\'s daily roster as a guest, ' +
          'and the HR portal will attribute her work to her home store. Hours stay with her home branch for payroll.' +
        '</div>' +
      '</div>' +
      '<div style="max-width:680px;margin:14px auto 0;display:flex;flex-direction:column;gap:10px">' +
        '<input id="borrow-search" type="search" autocomplete="off" placeholder="Search by name or EC…" ' +
          'class="input" style="padding:11px 14px;font-size:15px;border:2px solid var(--pink-200);border-radius:12px">' +
        '<div id="borrow-today-strip" style="font-size:12px;color:var(--gray-600)"></div>' +
        '<div id="borrow-results" style="display:flex;flex-direction:column;gap:8px"></div>' +
      '</div>' +
      '<div style="max-width:680px;margin:18px auto 0;text-align:center">' +
        '<button id="borrow-back" type="button" style="padding:9px 18px;background:#fff;color:var(--pink-700);border:2px solid var(--pink-200);border-radius:10px;font-weight:700;font-size:13px;cursor:pointer">← Back to home</button>' +
      '</div>'
    );

    document.getElementById("borrow-back").onclick = function () { renderManagerLanding(); };

    var listLoading = document.getElementById("borrow-results");
    listLoading.innerHTML = '<div style="color:var(--gray-500);font-style:italic;padding:14px;text-align:center">Loading staff list…</div>';
    ...
    try {
      ...
      allStaff = loaded[0] || [];
      todaysLoans = loaded[1] || [];
    } catch (e) {
      listLoading.innerHTML = '<div style="color:#7f1d1d;padding:14px">Could not load staff: ' + esc(String((e && e.message) || e)) + '</div>';
      return;
    }

    // Build the "already borrowed in" strip so the manager sees who's already here today.
    var incomingToday = todaysLoans.filter(function (l) { return l && l.toBranch === thisBranch && l.fromBranch !== thisBranch; });
    var stripEl = document.getElementById("borrow-today-strip");
    ...
    // Eligible pool: techs at other branches, who aren't already loaned IN here
    // today (no point picking someone who's already on the kiosk).
    ...
    var eligible = allStaff.filter(function (s) {
      if (!s || !s.employee_code) return false;
      if (s.branch === thisBranch) return false;
      if (alreadyIncomingEcs[s.employee_code]) return false;
      return true;
    });

    var inp     = document.getElementById("borrow-search");
    var results = document.getElementById("borrow-results");
    ...
    function renderResults() {
      ...
      if (matches.length === 0) {
        results.innerHTML = '<div style="color:var(--gray-500);font-style:italic;padding:14px;text-align:center">' +
          (q === "" ? "No staff available to borrow." : "No matches for &quot;" + esc(q) + "&quot;.") +
          '</div>';
        return;
      }
      results.innerHTML = matches.map(function (s) {
        return (
          '<div class="borrow-row" data-ec="' + esc(s.employee_code) + '" data-id="' + esc(s.id || "") + '" data-name="' + esc(s.name || "") + '" data-branch="' + esc(s.branch || "") + '" ' +
            ...
            '<div style="flex:1;min-width:0">' +
              ...
              '</div>' +
            '</div>' +
            '<button type="button" class="borrow-btn" style="background:var(--pink-700);color:#fff;border:none;border-radius:8px;padding:9px 16px;font-weight:700;font-size:13px;cursor:pointer">Borrow</button>' +
          '</div>'
        );
      }).join("");
    }
    ...
    results.addEventListener("click", async function (e) {
      var btn = e.target.closest && e.target.closest(".borrow-btn");
      if (!btn) return;
      var row = btn.closest(".borrow-row"); if (!row) return;
      var ec     = row.dataset.ec || "";
      var name   = row.dataset.name || "";
      var branch = row.dataset.branch || "";
      if (!ec) return;
      var ok = window.confirm("Borrow " + (name || ec) + " from " + branch + " for today?\n\nShe'll appear on today's roster here and HR will track her hours back to " + branch + ".");
      if (!ok) return;
      ...
      var loan = {
        ...
        fromBranch: branch,
        toBranch: thisBranch,
        note: "Walk-in via " + (cfg.branchDisplayName || thisBranch) + " kiosk",
      ...
      try {
        ...
        // Refresh the "already here today" strip
        renderBorrowTech();
      } catch (err) {
