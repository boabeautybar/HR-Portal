...
(function () {
  ...
  async function addManualKioskCheckin(branch, ec, ymd, status, note, markedBy) {
    ...
    // Also stamp the attendance grid so it shows on the Attendance sheet
    // straight away — mirroring what the kiosk does when a manager tags a
    // status. The attendance grid is keyed by the cycle's START month
    // (boa_att_<branch>_<startYm>): a cycle runs the 25th of month M → 24th of
    // M+1 and is stored under month M, so a date on/after the 25th uses its own
    // month, while the 1st–24th belong to the PREVIOUS month's cycle. The grid
    ...
    try {
      ...
      var ay = p[0], am = p[1];
      if (p[2] < 25) { am = p[1] - 1; if (am < 1) { am = 12; ay = p[0] - 1; } }
