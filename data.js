...
(function () {
  ...
  // ---------- Early-leave sidecar (boa_early_<branch>_<ym>) ----------
  // Written by the check-in app's "Left early" feature. Shape:
  //   { [dayKey]: { [employee_code]: { hours, recordedAt, recordedBy } } }
  // The HR portal subtracts these hours from each tech's day total on the
  // attendance grid. dayKey is the day-of-month string ("1".."31") matching
  // the keys used by the attendance grid for this cycle.
  async function loadEarlyLeaves(branch, ym) {
    var res = await sb.from("app_state").select("value").eq("key", "boa_early_" + branch + "_" + ym).maybeSingle();
